<template>
  <div>
    <div class="positionManaTool">
      <el-input v-model="role.authority" placeholder="请输入角色英文名" size="small">
        <template slot="prepend">ROLE_</template>
      </el-input>
      <el-input v-model="role.authorityName" placeholder="请输入角色中文名" size="small" @keydown.enter.native="doAddRole"></el-input>
      <el-button type="primary" icon="el-icon-plus" size="mini" @click="doAddRole">添加角色</el-button>
    </div>
    <!-- 手风琴 -->
    <div class="positionManaMain">
      <el-collapse v-model="activeName" accordion @change="change">
        <el-collapse-item :title="r.authorityName" :name="r.id" v-for="(r,index) in roles" :key="index">
...
export default {
  ...
  methods: {
    // 删除角色
    doDeleteRole(role){
      this.$confirm('此操作将永久删除[' + role.authorityName + ']角色, 是否继续?', '提示', {
    ...
    // 添加角色
    doAddRole(){
      if (this.role.authority && this.role.authorityName) {
